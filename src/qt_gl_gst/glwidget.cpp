    ...
    m_brickProg(this)
{
    ...
    // Video pipeline
    // This loop sets up the number of video pipelines and is constricted by the number of arguments
    // that are passed into the program via command line.
...
void GLWidget::loadVideoSlot()
{
    ...
    int lastVidDrawn = this->m_vidTextures.size() - 1;
    std::cout << "junk before video file select" << std::endl;
    ...
    std::cout << "junk after video file select" << std::endl;
    if(newFileName.isNull() == false)
    {   std::cout << "junk" << std::endl;

        std::cout << lastVidDrawn << std::endl;
//        std::cout << this->size() << std::endl;
        std::cout << m_videoLoc.size() << std::endl;

        this->m_videoLoc[lastVidDrawn] = newFileName;
        std::cout << "junk after" << std::endl;
        //this->m_vidPipelines[lastVidDrawn]->setChooseNewOnFinished();
        this->m_vidPipelines[lastVidDrawn]->Stop();
        std::cout << "junk after vidpipelines" << std::endl;
    }

#ifdef HIDE_GL_WHEN_MODAL_OPEN
    this->resize(currentSize);
    std::cout << 'junk after resize' << std::endl;
